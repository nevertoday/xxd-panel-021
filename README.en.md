<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 021 project banner" width="1200">
</p>
...
021 reverses that logic:

```text
lock source facts → choose one horizontal or vertical pure-black rectangle → place most of the subject inside it → let exactly one defining feature break out → rebuild the subject with jittery broken lightly retraced black line and white negative form → finish with microtype along the edge, negative space, or breakout direction
```

If an unrelated photograph could replace the source without materially changing rectangle orientation, subject placement, breakout feature, contour, white negative form, or copy, the result is not 021.

## The 021 visual contract
...
- **Rough photocopy line:** black line varies slightly in weight, jitters, breaks locally, and allows limited retracing; smooth stable vector outlines fail.
...
- **No colour:** near-white paper, pure black, black line, and tiny grey planes only—no accent colour or gradient.
...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 021 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 021's adaptability. Their subjects, metaphors, palette, copy, and canvas ratios will never become generation references or defaults.
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 021 black-field breakout line design below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 021 black-field breakout line design right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `021.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore different field orientation, subject occupancy, breakout feature, photocopy-line rhythm, and type relation.
...
The image model creates the source-bound black field, subject occupancy, exactly one breakout feature, rough photocopy line, white negative form, tiny grey planes, and editorial type. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-021-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-021-prompt.en.md)
- [Original style brief](references/021-source.md)
