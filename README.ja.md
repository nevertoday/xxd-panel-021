...
リポジトリには将来の作例用に [`assets/examples/`](assets/examples/) を用意しています。プロジェクト作者が確認した 021 の完成作品だけを追加し、それまでは別スタイルの投稿や画像を代用しません。

将来の作例は 021 の応用範囲を示すだけで、主体、比喩、配色、文言、画角が生成参照や既定値になることはありません。
...
| モード | サイズ方針 | 成果物 |
| --- | --- | --- |
| `top-bottom` | 元画像適応 | 上に完全な元写真、下に 021 の黒い領域を越境する線画。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `left-right` | 元画像適応 | 左に完全な元写真、右に 021 の黒い領域を越境する線画。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `design-only` | 元画像適応 | 元写真を見せず、変化デザインだけを表示。元画像の比率と寸法を維持 |
| `wallpaper-pack` | 端末別4サイズ | スマートフォン、iPad、デスクトップ、子ども用ウォッチの PNG を個別出力 |

優先順位は、正確な指定ピクセル > 指定比率／用途 > 通常モードの元画像適応です。原稿 `021.md` の 3:4 は初期の制作画角であり、現在の暗黙の既定値ではありません。
...
- **独立4作品：** 各端末は元写真だけを参照し、矩形の向き、主体の占有率、越境する特徴、コピー線のリズム、文字関係をそれぞれ探れます。
...
画像モデルが写真に結びついた黒い領域、主体の占有、ちょうど一つの越境特徴、粗いコピー線、白いネガ形、微小な灰色面、編集文字を作ります。`scripts/compose_panel.py` は画布計画、厳密な 50/50 ラスター合成、最終寸法、監査だけを担当し、プログラム描画で作品を偽装しません。
...
- [中国語の完全プロンプト](references/xxd-panel-021-prompt.zh-CN.md)
- [英語の完全プロンプト](references/xxd-panel-021-prompt.en.md)
