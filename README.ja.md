<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 021 プロジェクトバナー" width="1200">
</p>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 021 作例 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 021 作例 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 021 作例 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 021 作例 8"></td>
  </tr>
</table>

## 追加 3:4 上下構成作例

上の 16:9 作例と同じ4点の独立素材を、完全な 3:4 上下構成として再生成しました。上部は現実の写真を保ち、下部は Panel 021 の原文プロンプトに従って再構成しています。英語コピーは各写真の内容・空気・含意だけから生成し、全点を個別に確認しました。

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 021 追加上下作例 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 021 追加上下作例 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 021 追加上下作例 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 021 追加上下作例 4"></td>
  </tr>
</table>
...
> 純黒矩形 · 主体の大半は内部 · 一つだけ越境 · 揺れるコピー線 · 白いネガ形と微小な灰色面

XXD Panel 021 は、Codex と互換 Agent のための画像生成 Skill です。写真から主体、輪郭、姿勢、動き、機能、関係を固定し、横長または縦長の純黒矩形を主領域にします。主体の大半をその内側に置き、最も識別的な構造を一つだけ自然に境界の外へ出します。
...
**Panel 021** は、原写真の核を保ちながら、次の視覚方向を目指す制作に向いています：**写真の主体を、ひとつの純黒矩形から越境させる**。まず用途と期待する結果を確認し、そこから構図、サイズ、文字、納品形式を決めます。
...
> XXD Panel 021 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
## すぐに判断：XXD Panel 021 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 写真の主体を、ひとつの純黒矩形から越境させる |
| **ひと目で分かる特徴** | 純黒矩形 · 主体の大半は内部 · 一つだけ越境 · 揺れるコピー線 · 白いネガ形と微小な灰色面 |
...
```text
元写真の事実を固定 → 横長または縦長の純黒矩形を一つ選ぶ → 主体の大半を内部へ置く → 決定的な特徴を一つだけ越境させる → 揺れ、途切れ、わずかな重描を含む黒線と白いネガ形で再構成 → 境界、ネガ形、越境方向に沿う微小文字で仕上げる
```

無関係な写真に替えても矩形の向き、主体位置、越境する特徴、輪郭、白いネガ形、文言がほぼ成立するなら、それは 021 ではありません。
...
- **粗いコピー線：** 黒線はわずかに太さが変わり、震え、局所的に途切れ、限定的に描き直されます。滑らかで安定したベクター線は不可です。
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 021 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-021 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-021 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-021 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-021 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 021 を適用し、共通設定は一度だけ解決
/xxd-panel-021 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを `~/.claude/skills/xxd-panel-021` にリンクできます。インストール後に Agent セッションを再起動してください。
...
- [中国語ランタイムアダプター](references/xxd-panel-021-prompt.zh-CN.md)
- [英語ランタイムアダプター](references/xxd-panel-021-prompt.en.md)
