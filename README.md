<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 021 项目横幅" width="1200">
</p>
...
> 纯黑矩形 · 主体大部入场 · 一个特征越界 · 抖动复印线 · 白色负形与微灰面

XXD Panel 021 是一个面向 Codex 与兼容 Agent 的图像生成 Skill。它先从照片中锁定身份、轮廓、姿态、动作、功能与关系，再用一个横向或纵向的纯黑矩形建立主场：主体的大部分留在黑色域内，只有一个最具识别度的结构自然穿出边界。

主体以粗细略变、轻微抖动、局部断裂和少量重复描线的墨笔／针管笔／复印线稿出现；白色负形承担主要识别，极少灰面只用于辅助结构。大面积白纸、不对称位置、局部裁切与小型编辑文字，让越界动作既幽默又克制。
...
```text
锁定源图事实 → 选择横向或纵向纯黑矩形 → 让主体大部分进入黑色域 → 只让一个决定性特征越界 → 用抖动、断裂、少量复描的黑线与白色负形重建主体 → 让微文字沿边界、负形或越界方向完成构图
```

如果换成一张无关照片，矩形方向、主体位置、越界特征、轮廓、白色负形与文字关系仍然成立，这张图就不属于 021。
...
- **粗粝复印线：** 黑线粗细略变、轻微抖动、局部断裂并允许少量复描；拒绝稳定顺滑的矢量描边。
- **白负形＋微灰面：** 白色负形建立识别，灰色只占极小面积；主体绝不能变成纯黑剪影。
- **绝对无彩色：** 近白纸、纯黑色域、黑线和极少灰面之外不出现任何色彩或渐变。
- **不对称与局部裁切：** 大面积白纸和偏置构图制造张力，辅助线面保持极少，不发展成复杂场景。
- **编辑文字参与越界：** 一个短标题＋1–3 组微文字进入矩形边缘、白色负形、越界方向或主体轮廓，而不是另贴标签。

## 样张 · 来自 X

> [小小东（@xiaoxiaodong01）](https://x.com/xiaoxiaodong01/status/2090264725116551641) · 2026-08-20<br>
...
<p align="center"><a href="https://x.com/xiaoxiaodong01/status/2090264725116551641">查看原推文与完整提示词 →</a></p>

这些样张用于展示 021 的美学动机，不会把样张中的主体、构图、配色、文案或旧画幅变成生成参考或当前默认值。
...
| 模式 | 尺寸逻辑 | 成品 |
| --- | --- | --- |
| `top-bottom` | 源图自适应 | 上方完整原图，下方 021 黑色域越界线稿设计；两块都保持原图完整尺寸，严格 50/50 |
| `left-right` | 源图自适应 | 左侧完整原图，右侧 021 黑色域越界线稿设计；两块都保持原图完整尺寸，严格 50/50 |
| `design-only` | 源图自适应 | 只显示变化设计，不显示原照片；沿用原图比例和尺寸 |
| `wallpaper-pack` | 四种设备尺寸 | 分别输出手机、iPad、电脑、儿童手表四张 PNG |

用户精确尺寸 > 指定比例或用途 > 普通模式源图自适应。原始 `021.md` 里的 3:4 只是一开始的创作画幅，不会被写成当前 Skill 的静默默认值。
...
- **四张独立：** 每张只参考原照片，可以分别探索不同的矩形方向、主体入场比例、越界特征、复印线节奏与文字关系。
...
日本版使用自然日语，韩国受众使用自然韩语与正确空格，英国版使用英式英语，阿拉伯语版默认使用自然的现代标准阿拉伯语和真正的从右到左排版。字体会在当地文字系统中寻找克制的艺术出版物小字与粗粝复印线相匹配的自然等价物，不会把拉丁排版规则生硬套过去。

## 精确拼版交给代码，作品交给图像生成

图像模型负责源图绑定的黑色域、主体入场、恰好一个越界特征、粗粝复印线、白色负形、微灰面和编辑文字。`scripts/compose_panel.py` 只负责画布规划、精确 50/50 位图拼合、最终尺寸和审计，不会用程序绘图伪造成品。
...
- [Skill 工作流](SKILL.md)
- [中文完整提示词](references/xxd-panel-021-prompt.zh-CN.md)
- [英文完整提示词](references/xxd-panel-021-prompt.en.md)
- [原始风格提示词](references/021-source.md)
